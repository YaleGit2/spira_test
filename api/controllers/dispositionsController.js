...
exports.dispositions = async (req, res) => {
  try {
    ...
    queryString.docType = 'disposition';
    ...
    if (typeof resultFabric.results !== undefined) {
      ...
      if (resultFabric.results.length > 0) {
        console.log(resultFabric.results.length, ' disposition returned');
...
exports.dispositionsResource = async (req, res) => {
  try {
    const contract = req.blockchain.epcisContract;
    res.set({
      'GS1-EPCIS-Version': '2.0',
    ...
    const dispositionParam = req.params.disposition;
    const [queryCheck, queryCheckError] = checker.checkQueryParameter(
      req.headers
    );
    if (!queryCheck) {
    ...
    var queryString = {};
    queryString.docType = 'disposition';
    queryString.voc = dispositionParam;
    let mangoQueryString = {};
    mangoQueryString.selector = queryString;
    mangoQueryString = JSON.stringify(mangoQueryString);
...
exports.dispositionsEvent = (req, res) => {
  //console.log(req.params);
  if (req.query.EQ_disposition) {
    return responseUtil.response400(
      res,
      'EQ_disposition should not be included in the query parameters'
    );
  }
  req.query.EQ_disposition = '["' + req.params.disposition + '"]';
