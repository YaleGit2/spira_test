...
exports.handleWebSocketRequest = async (ws, request) => {
  ...
  var scheduled_job;
  if (typeof queryString.minRecordTime !== 'undefined') {
    subscription.minRecordTime = queryString.minRecordTime;
  }
  if (typeof queryString.reportIfEmpty !== 'undefined') {
    subscription.reportIfEmpty = queryString.reportIfEmpty;
  }
  ...
  if (typeof queryString.stream !== 'undefined') {
    ...
    typeof queryString.second !== 'undefined' ||
    typeof queryString.dayOfWeek !== 'undefined' ||
    typeof queryString.minute !== 'undefined' ||
    typeof queryString.hour !== 'undefined' ||
    typeof queryString.dayOfMonth !== 'undefined' ||
    typeof queryString.month !== 'undefined'
  ) {
    subscription.schedule = url.parse(request.url, false).query;

    if (typeof queryString.second !== 'undefined') {
      second = ' ' + queryString.second + ' ';
    }
    if (typeof queryString.minute !== 'undefined') {
      minute = ' ' + queryString.minute + ' ';
    }
    if (typeof queryString.hour !== 'undefined') {
      hour = ' ' + queryString.hour + ' ';
    }
    if (typeof queryString.dayOfMonth !== 'undefined') {
      dayOfMonth = ' ' + queryString.dayOfMonth + ' ';
    }
    if (typeof queryString.month !== 'undefined') {
      month = ' ' + queryString.month + ' ';
    }
    if (typeof queryString.dayOfWeek !== 'undefined') {
      dayOfWeek = ' ' + queryString.dayOfWeek + ' ';
    }
  ...
  ws.on('close', async function () {
    if (typeof scheduled_job !== 'undefined') {
      scheduled_job.stop();
    }
    try {
...
async function serveSubscription(ws) {
  ...
  const subscriptions =
    await subscriptionController.getSubscriptionWithSubscriptionID(
      ws.subscription.subscriptionID
    );
  if (subscriptions.length > 0) {
    const queries = await subscriptionController.getQueryWithQueryName(
