...
exports.userLogin = async (req, res) => {
  try {
    ...
    if (stat === 'username') {
      return res
        .status(401)
        .json({ error: 'Invalid credentials, Incorect username' });
    }
    if (stat === 'password') {
      return res
        .status(401)
        .json({ error: 'Invalid credentials, Incorect password' });
    }
    if (stat === 'waiting') {
      return res
        .status(401)
        .json({ error: 'Invalid credentials, Approval is waiting' });
    }
...
exports.userRegister = async (req, res) => {
  try {
    ...
    if (userNameExist) {
      return res
        .status(401)
        .json({ error: 'Username exist, use a different username' });
    }
    ...
    couch
      .insert(userdb, normalUser)
      .then(() => {
        console.log('User registered successfully');
        return res
          .status(202)
          .json('User registered successfully need to for approval ');
      })
      .catch((error) => {
        ...
        ({ data, headers, status }) => {
          if (data.docs.length > 0) {
            ...
            if (typeof req.body.newEmail !== 'undefined') {
              userTobeUpdated.eml = req.body.newEmail;
            }

            couch.update(userdb, userTobeUpdated).then(
              ({ data, headers, status }) => {
      ...
      couch.mango(userdb, query).then(
        ({ data, headers, status }) => {
          if (data.docs.length > 0) {
            ...
            console.log("hrerrrrrr");
            couch.update(userdb, userTobeApproved).then(
              ({ data, headers, status }) => {
                //console.log(data);
                return res.status(203).json({ Status: 'user updated' });
              },
      ...
      couch.mango(userdb, query).then(
        ({ data, headers, status }) => {
          if (data.docs.length > 0) {
            ...
            couch.del(userdb, user._id, user._rev).then(
              ({ data, headers, status }) => {
