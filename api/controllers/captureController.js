  ...
  try {
    ...
    if (!valStatus) {
      ...
      if (epcisDocument.epcisHeader) {
        if (epcisDocument.epcisHeader.epcisMasterData) {
          if (epcisDocument.epcisHeader.epcisMasterData.vocabularyList) {
            const vacList =
              epcisDocument.epcisHeader.epcisMasterData.vocabularyList;
              ...
              if (vacList[vlkey].vocabularyElementList) {
                ...
                for (const vEkey in vocElementList) {
                  ...
                  if (typeof vocElement.id !== 'undefined') {
                    ...
                    console.log(
                      '*** Result: committed from masterdata',
                      JSON.parse(response)
                    );
                  }
                }
      ...
      if (req.query.hasOwnProperty('GS1-Capture-Error-Behaviour')) {
        if (
          ['rollback', 'proceed'].includes(
            req.query['GS1-Capture-Error-Behaviour']
          )
        ) {
      ...
      let eventList;
      if (
        epcisDocument.type == 'EPCISQueryDocument' ||
        epcisDocument.isA == 'EPCISQueryDocument'
      ) {
      ...
      if (masterDataCount > 0) {
        ...
        if (eventList) {
          if (eventList.length == 0) {
            return res
              .status(202)
              .send('Successfully received ' + masterDataresposneText);
          }
        }
      }
      ...
      if (typeof eventList !== 'undefined') {
        if (eventList.length > queryConfig.maxEventCapture) {
          return res
            .status(416)
            .send(' Number of events per single capture excedded the limit ');
        }
      }
      ...
      captureAsync(eventList, contextG, req.blockchain.epcisContract)
        .then((result) => {
      ...
      const resposneText =
        'Successfully received ' + eventList.length + ' EPCIS events';
      res.set({
...
async function captureAsync(eventList, contextG, contract) {
  for (let i = 0; i < eventList.length; i++) {
    ...
    const responseEvent = await contract.submitTransaction(
      'CaptureEvent',
      args
    );
    console.log(
      '*** Result: committed evet capture',
      JSON.parse(responseEvent)
    );
  }
}
