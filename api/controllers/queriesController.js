...
async function getQueryWithQueryName(queryName, contract) {
  var queryString = {};
...
exports.queriesPost = async (req, res) => {
  try {
    ...
    let resultTransction = await getQueryWithQueryName(queries.name, contract);
    ...
    if (resultTransction.length > 0) {
      return responseUtil.response400(
        res,
        'Query Name already exist; Use a different unique query name'
      );
    } else {
...
exports.queriesQueryNameGet = async (req, res) => {
  try {
    ...
    let resultTransction = await getQueryWithQueryName(queryName, contract);
...
exports.queriesQueryNameEventGet = async (req, res) => {
  try {
    ...
    if (resultTransction.length > 0) {
      ...
      queryUtility.getQueryResultByQueryName(
        req,
        res,
        resultTransction[0].Record.query
      );
    } else {
      return responseUtil.response404(res, '');
    }
...
exports.queriesQueryNameDelete = async (req, res) => {
  try {
    ...
    let queryResultTransction = await getQueryWithQueryName(
      queryNameVal,
      contract
    );
